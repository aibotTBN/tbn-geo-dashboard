...
import { RagReadinessCard } from '@/components/geo/rag-readiness'
import { GoogleAiOverviewCard } from '@/components/geo/google-ai-overview'
...
interface DiagnosisData {
  ...
  ragReadiness?: { score: number; max: number; details: any } | null
  googleAiOverview?: { brand_cited: boolean; brand_mentioned_in_text: boolean; total_sources: number; brand_urls: string[]; cited_domains: string[]; search_queries: string[]; brand_support_texts: string[] } | null
}
    ...
    try {
      ...
      if (data.result?.geo_score !== undefined) {
        setDiagnosis({
          ...
          ragReadiness: data.result.rag_readiness || null,
          googleAiOverview: data.result.google_ai_overview || null,
        })
  ...
  return (
    <>
      ...
      <div className="p-6 space-y-6">
        ...
        {/* Google AI Overview — Search-Grounded Visibility */}
        {diagnosis?.googleAiOverview && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Wird Ihre Website in Googles KI-gestützten Suchergebnissen als Quelle zitiert?
              </CardDescription>
            </CardHeader>
            <CardContent>
              <GoogleAiOverviewCard data={diagnosis.googleAiOverview} />
            </CardContent>
          </Card>
        )}

        {/* RAG Readiness Score */}
        {diagnosis?.ragReadiness && (
          <Card>
            <CardHeader>
              <CardTitle className="text-lg flex items-center gap-2">
                🧩 RAG-Readiness Score
            ...
            <CardContent>
              <RagReadinessCard data={diagnosis.ragReadiness} />
            </CardContent>
          </Card>
        )}
        ...
        {diagnosis && reportJsonRaw && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Die tatsächlichen Antworten von ChatGPT, Claude, Gemini, Perplexity und Google AI auf einheitliche branchenspezifische Fragen
              </CardDescription>
            </CardHeader>
            <CardContent>
              <LlmAnswers
